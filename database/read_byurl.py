...
class SpiderUrl:
    def __init__(self,url=None,stock_code=None,years=None):
        ...
        self.years = DEFAULT_YEARS if not years else years

    def get_from_url(self,url=None):
        self.req = requests.get(url,headers=self.headers,timeout=120)
    ...
    def get_belance(self,years=None):
        data = self.get_from_url(self.belance_url)
        data = self.filter_by_years(data,years)
        return data

    # return type：DataFrame
    def get_benefit(self,years=None):
        data = self.get_from_url(self.benefit_url)
        data = self.filter_by_years(data,years)
        return data

    # return type：DataFrame
    def get_cash(self,years=None):
    ...
    def gen_data_bytype(cls,stock_code,type=None,years=None):
        ...
        if type=='b':
            return obj.get_belance(years)
        elif type=='p':
            return obj.get_benefit(years)
        elif type=='c':
            return obj.get_cash(years)
        else:
            try:
...
if __name__=="__main__":
    b = SpiderUrl.gen_data_bytype('002350',type='b')
    p = SpiderUrl.gen_data_bytype('002350',type='p')
    c = SpiderUrl.gen_data_bytype('002350',type='d')
    print(b.head())
    print(p.head())
    print(c.head())